...
namespace Vip.Dialogs
{
    ...
    ///         <see cref="System.Windows.Forms.OpenFileDialog.ShowReadOnly" />,
    ...
    public class VistaSaveFileDialog : VistaFileDialog
    {
        ...
        ///     Creates a new instance of <see cref="VistaSaveFileDialog" /> class.
        /// </summary>
        /// <param name="forceDownlevel">
        ///     When <see langword="true" />, the old style common file dialog will always be used even if
        ///     the OS supports the Vista style.
        /// </param>
        ...
        /// </summary>
        /// <value>
        ///     <see langword="true" /> if the dialog box prompts the user before creating a file if the user specifies a file name
        ///     that does not exist;
        ///     <see langword="false" /> if the dialog box automatically creates the new file without prompting the user for
        ...
        /// </summary>
        /// <value>
        ///     <see langword="true" /> if the dialog box prompts the user before overwriting an existing file if the user
        ///     specifies a file
        ///     name that already exists; <see langword="false" /> if the dialog box automatically overwrites the existing file
        ///     without
        ...
        ///     <see cref="VistaFileDialog.FileName" /> is <see langword="null" /> or an empty
        ///     string.
        /// </exception>
        ...
        protected override void OnFileOk(CancelEventArgs e)
        {
            ...
            if (DownlevelDialog == null)
            {
                ...
                {
                    PromptUser(
                        ComDlgResources.FormatString(ComDlgResources.ComDlgResourceId.FileNotFound,
                            Path.GetFileName(FileName)), MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                    e.Cancel = true;
                    return;
                }
                if (CreatePrompt && !File.Exists(FileName))
                    if (!PromptUser(
                        ComDlgResources.FormatString(ComDlgResources.ComDlgResourceId.CreatePrompt,
                            Path.GetFileName(FileName)), MessageBoxButtons.YesNo, MessageBoxIcon.Exclamation))
                    {
                        e.Cancel = true;
                        return;
                    }
