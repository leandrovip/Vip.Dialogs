...
namespace Vip.Dialogs
{
    ...
    ///         Vista it is Segoe UI 9pt. By setting the <see cref="UseSystemFont" /> property to <see langword="true" /> for a
    ///         form,
    ...
    public class ExtendedForm : Form
    {
        ...
        ///     </para>
        ///     <note>
        ///         It is strongly recommended to test your application with different font and DPI settings if you set this
        ///         property
        ...
        /// </summary>
        /// <value>
        ///     <see langword="true" /> when the form can be dragged using the client area glass; otherwise,
        ///     <see langword="false" />.
        ///     The default value is <see langword="false" />.
        /// </value>
        /// <remarks>
        ///     This property has no effect on operating systems older than Windows Vista, when the Desktop Window Manager is
        ///     disabled
        ...
        [DefaultValue(true)]
        public bool AllowGlassDragging { get; set; } = true;
        ...
        protected override void OnPaintBackground(PaintEventArgs pevent)
        {
            if (DesignMode || Glass.IsDwmCompositionEnabled)
                if (DesignMode)
                    using (var brush = new HatchBrush(HatchStyle.OutlinedDiamond, Color.SkyBlue, BackColor))
                    {
        ...
        protected override void WndProc(ref Message m)
        {
            ...
            switch (m.Msg)
            {
                case NativeMethods.WM_NCHITTEST:
                    if (AllowGlassDragging && m.Result == new IntPtr((int) NativeMethods.HitTestResult.Client) &&
                        Glass.IsDwmCompositionEnabled)
                        ...
                        {
                            var p = new Point((int) m.LParam & 0xffff, (int) m.LParam >> 16);
                            p = PointToClient(p);
                            if (p.X < _glassMargin.Left || p.X > ClientSize.Width - _glassMargin.Right ||
                                p.Y < _glassMargin.Top || p.Y > ClientSize.Height - _glassMargin.Bottom)
                                m.Result = new IntPtr((int) NativeMethods.HitTestResult.Caption);
                        }
                    break;
        ...
        ///     A <see cref="BoundsSpecified" /> value that specifies the bounds of the control to use when
        ///     defining its size and position.
        /// </param>
        protected override void ScaleControl(SizeF factor, BoundsSpecified specified)
        {
            var width = factor.Width;
            var newMargin = GlassMargin;
            if (width != 1f)
            {
                if (newMargin.Left > 0)
        ...
        private void PaintGlassArea(PaintEventArgs pevent, Brush brush)
        {
            ...
            {
                var rect = new Rectangle(_glassMargin.Left, _glassMargin.Top, ClientSize.Width - _glassMargin.Right,
                    ClientSize.Height - _glassMargin.Bottom);
                ...
                if (_glassMargin.Bottom != 0)
                    pevent.Graphics.FillRectangle(brush,
                        new Rectangle(0, ClientSize.Height - _glassMargin.Bottom, ClientSize.Width, ClientSize.Height));
            }
        }

        private void SystemEvents_UserPreferenceChanged(object sender, UserPreferenceChangedEventArgs e)
        {
            if (e.Category == UserPreferenceCategory.Window && UseSystemFont)
                Font = SystemFonts.IconTitleFont;
        }
    }
}